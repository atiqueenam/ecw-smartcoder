...
(function () {
    ...
    // Medicaid, Medicare, Wellcare, Fidelis, UHC (incl. UMR), NYCE PPO
    // and Clover Health — MetroPlus allowed. Aligned here so the annual
    // screening G-codes (G0444/G0442) and 99401 use one consistent payer
    // exclusion list rather than two that can drift apart.
    // Unknown/unparsed insurance still defaults eligible (as in Bronx).
    function annualGCodesEligible(insurance) {
        return !isHasnayenCounselingBlockedIns(insurance);
    }
    ...
    function computeAnalysis() {
        ...
        // Rule 6 adds the literal "Televisit" caption alongside Bronx's
        // CON/TEL spellings — all three resolve to the same rule-6 row.
        const isTelevisitNote = ['con', 'tel', 'televisit'].includes(getVisitType().toLowerCase().trim());
        ...
        // Hasnayen: plain age check ONLY — kept for 65+, removed if under
        // 65. No televisit-vs-normal (1157F/1158F) or pain-vs-no-pain
        ...
        // Rule 30: 99407 gets the identical downstream treatment as 99406.
        const EMPIRE_TOBACCO_CODES = ['99406', '99407', '1000F', '1036F', 'G9275', 'G9276'];
        if (isEmpire) {
        ...
        // ---- Obesity ICD (E66.3/E66.9/E66.01/E66.09): Hasnayen rule 26 ----
        // Correction-only, exactly as in Bronx: never added from nothing
        // here (the Obesity Counseling quick action is what introduces
        // E66.9) — but if one is already on the chart (added by the
        // practice, a prior visit, etc.), it gets corrected or removed to
        // match the current BMI / BMI percentile.
        //
        // Rule 26 ranges:
        //   18+   BMI 25-29.9  -> E66.3   (we NEVER add this ourselves —
        //                                  only verify an existing one is
        //                                  right for the actual BMI)
        //         BMI 30+      -> E66.9
        //         BMI 40+      -> E66.01
        //   <18   percentile 95%+   -> E66.9
        //         percentile 85-94% -> E66.3
        // E66.09 is no longer a target code (Bronx used it for BMI 50+);
        // it stays in the "recognized obesity ICD" list so an existing one
        // is still corrected/removed rather than left orphaned.
        {
            const OBESITY_ICD_CODES = ['E66.3', 'E66.9', 'E66.01', 'E66.09'];
            const currentObesityEntries = getICDRows().filter(e => OBESITY_ICD_CODES.includes(e.code.toUpperCase()));
            if (currentObesityEntries.length && age != null) {
                let correctObesityCode = null;
                let basisLabel = '';
                if (age >= 18) {
                    const bmiNumObesity = parseFloat(bmi);
                    if (!isNaN(bmiNumObesity)) {
                        basisLabel = `BMI ${bmiNumObesity}`;
                        if (bmiNumObesity >= 40) correctObesityCode = 'E66.01';
                        else if (bmiNumObesity >= 30) correctObesityCode = 'E66.9';
                        else if (bmiNumObesity >= 25) correctObesityCode = 'E66.3';
                    }
                } else {
                    const pct = parseFloat(bmiPercentile);
                    if (!isNaN(pct)) {
                        basisLabel = `BMI percentile ${pct}%`;
                        if (pct >= 95) correctObesityCode = 'E66.9';
                        else if (pct >= 85) correctObesityCode = 'E66.3';
                    }
                }

                // No usable BMI/percentile at all -> can't judge, leave the
                // chart alone (never delete on missing data).
                if (basisLabel) {
                    if (correctObesityCode) {
                        const hasCorrectObesity = currentObesityEntries.some(e => e.code.toUpperCase() === correctObesityCode);
                        // Rule 26: E66.3 is never added by us — if that's
                        // the correct code and it isn't there, we only
                        // remove the wrong ones, we don't add E66.3.
                        if (!hasCorrectObesity && correctObesityCode !== 'E66.3') {
                            toAdd.push({ code: correctObesityCode, reason: `${basisLabel} — correct obesity code`, kind: 'icd' });
                        }
                        currentObesityEntries.forEach(e => {
                            if (e.code.toUpperCase() !== correctObesityCode) {
                                toDelete.push({ code: e.code, row: e.row, kind: 'icd', reason: `Wrong obesity code for ${basisLabel} (should be ${correctObesityCode})` });
                            }
                        });
                    } else {
                        // Below the lowest band — none of the obesity codes apply.
                        currentObesityEntries.forEach(e => {
                            toDelete.push({ code: e.code, row: e.row, kind: 'icd', reason: `${basisLabel} — below the overweight/obesity range, obesity code not applicable` });
                        });
                    }
                }
            }
        ...
        // ---- Office Visit E&M code (Hasnayen rules 1, 6, 9, 12, 17, 27) ----
        // Rule 1: 99213 is the default add when NO office-visit code
        // exists on the chart yet. If ANY office-visit code is already
        // present (put there by the practice/doctor), it is NEVER removed
        // or replaced — we simply don't add one. That's a deliberate
        // departure from Bronx, which force-corrected a mismatched
        // office-visit code; for Hasnayen the practice's own code always
        // wins.
        //
        ...
        // (CC-is-refill-only -> 99212) is deliberately NOT ported.
        //
        // Rule 9: commercial-insurance handling follows Getwell, not
        // Bronx — when the payer is commercial (Aetna, Cigna, BCBS/Blue
        // Cross, Empire, UHC/UMR/Oxford) AND a Preventive code is on the
        // chart, no office-visit E&M code is billable alongside it.
        // NYCE PPO gets the same treatment (Bronx already had that half).
        const visitType = getVisitType();
        const visitInfo = hasnayenVisitTypeInfo(visitType);
        const visitCategory = classifyVisitType(visitType);
        const isCommercialInsForOV = isHasnayenCommercialInsurance(insurance);
        const blockOfficeVisitForCommercialPreventive =
            (isCommercialInsForOV || isNycePPOForOV) && hasPreventiveVisit;

        if (blockOfficeVisitForCommercialPreventive) {
            // Getwell's commercial+preventive block: any office-visit code
            // present is removed and none is proposed.
            currentRows.forEach(r => {
                if (OFFICE_VISIT_EM_CODES.includes(r.code) && !toDelete.some(d => d.code === r.code)) {
            ...
            // New-patient E/M code list (rule 1): if the practice already
            // put ANY code from this list on the chart, theirs stands and
            // we add nothing.
            const NEW_PATIENT_EM_CODES = ['99201', '99202', '99203', '99204', '99205'];
            const ESTABLISHED_EM_CODES = ['99211', '99212', '99213', '99214', '99215'];

            const practiceNewPatientCode = NEW_PATIENT_EM_CODES.find(c => currentCodes.has(c));
            const practiceEstablishedCode = ESTABLISHED_EM_CODES.find(c => currentCodes.has(c));
            const existingOfficeVisitCode = practiceNewPatientCode || practiceEstablishedCode;
            ...
            if (visitCategory === 'new') {
                // Rule 1: 99203 from the new-patient E/M list, but only
                // when the practice hasn't already added one from that
                // same list.
                ovIsNewPatient = true;
                if (!practiceNewPatientCode) {
                    ovCode = '99203';
            ...
            // CLIENT-CONFIRMED: 99214 is never allowed on a Fidelis Care
            // chart under any circumstance. This is the ONE exception to
            // rule 1's "never remove an existing office-visit code": when
            // the payer is Fidelis and the practice already put 99214 on
            // the chart, we delete it and put 99213 in its place. Every
            // other insurance keeps rule 1's protection untouched.
            const fidelisExisting99214 = isFidelisIns(insurance) && currentCodes.has('99214');
            ...
            // Rule 1: never remove or replace an office-visit code the
            // practice already put on the chart. So the ONLY thing left to
            // do is add ours when there is no office-visit code at all.
            // Nothing here ever pushes an office-visit code to toDelete
            // (except the Fidelis 99214 override immediately above).
            if (ovCode && !existingOfficeVisitCode && !fidelisExisting99214) {
                toAdd.unshift({
                    code: ovCode,
                    reason: ovReason,
                    kind: 'em',
            ...
            // ---- 99214 vs TCM (99495/99496): downgrade to 99213 ----
            // 99214 is never billed together with a TCM code for this
            // client. If both are applicable — a TCM code is on the chart
            // and the effective office-visit code (whatever the logic
            // above landed on, including a practice-added 99214 that
            // rule 1 would otherwise leave untouched) is 99214 — use the
            // TCM code and downgrade the office visit to 99213 instead.
            // (Bronx and Getwell are the only two clients allowed to bill
            // 99214 and a TCM code together — this downgrade does not
            // apply to them.)
            const effectiveOvCodeForTcm = fidelisExisting99214 ? '99213' : (existingOfficeVisitCode || ovCode || null);
            const hasTCMForOvDowngrade = rawCPTCodesNow.includes('99495') || rawCPTCodesNow.includes('99496');
            if (hasTCMForOvDowngrade && effectiveOvCodeForTcm === '99214') {
                currentRows.forEach(r => {
                    if (r.code === '99214' && !toDelete.some(d => d.code === '99214')) {
                        toDelete.push({ code: '99214', row: r.row, kind: 'cpt', reason: 'TCM code (99495/99496) present — 99214 not billed with TCM, downgraded to 99213' });
                    }
                });
                if (!currentCodes.has('99213') && !toAdd.some(a => a.code === '99213')) {
                    toAdd.push({
                        code: '99213',
                        reason: 'TCM code present — 99214 downgraded to 99213',
                        kind: 'em',
                        emCategory: 'E/M SERVICES',
                        emIsNewPatient: false
                    });
        ...
        {
            const effectiveOvCodeFor99401 = currentRows.map(r => r.code).find(c => OFFICE_VISIT_EM_CODES.includes(c))
                || (toAdd.find(a => OFFICE_VISIT_EM_CODES.includes(a.code)) || {}).code
                || null;
            if (effectiveOvCodeFor99401 === '99214' && !toDelete.some(d => d.code === '99214')) {
        ...
        // ================= HASNAYEN-SPECIFIC ICD RULES =================
        // Rules 14, 15, 16, 18, 20, 21, 24, 28, 29. Bronx has no
        // equivalent code path for most of these (it never adds
        // T78.40XS / a dorsalgia code / E78.1 / K29.00 / cough / fever
        // ICDs at all), so they're implemented here as chart-level
        // replace/prune passes using the same toAdd/toDelete shape every
        // other rule in this function uses.
        {
            const hasnayenIcdEntries = getICDRows();
            const hasnayenIcdCodes = hasnayenIcdEntries.map(e => e.code.toUpperCase());
            ...
            // ---- Rules 28 + 29: immunization / pap-smear encounters ----
            // Rule 28: if an immunization (vaccine or injection admin) is
            // on the chart, Z23 is added/kept and no counseling code is
            // used. Rule 29: Q0091/G0101 mean Z12.4 is added and, again,
            // no counseling code — only preventive and immunization codes
            // may sit alongside. (The 25 modifier for both rules is
            // applied by al_applyHasnayen25Modifiers on the Link pass.)
            const HASNAYEN_IMMUNIZATION_CPTS = new Set([
                ...VACCINE_ADMIN_CODE_UNIVERSE,
                ...Object.keys(VACCINE_COMPONENT_MAP),
                ...COVID_VACCINE_CODES
            ]);
            const hasImmunizationNow = rawCPTCodesNow.some(c => HASNAYEN_IMMUNIZATION_CPTS.has(c)) ||
                Array.from(desired.keys()).some(c => HASNAYEN_IMMUNIZATION_CPTS.has(c));
            const PAP_SCREENING_CPTS = ['Q0091', 'G0101'];
            ...
            if (hasImmunizationNow) {
                for (let i = toDelete.length - 1; i >= 0; i--) {
                    if ((toDelete[i].code || '').toUpperCase() === 'Z23') toDelete.splice(i, 1);
                }
            }
            ...
            if (hasImmunizationNow || hasPapScreeningNow) {
                const counselingRow = currentRows.find(r => r.code === '99401');
                if (counselingRow && !alreadyQueuedForDelete('99401')) {
                    toDelete.push({
                ...
                for (let i = toAdd.length - 1; i >= 0; i--) {
                    if ((toAdd[i].code || '').toUpperCase() === '99401') toAdd.splice(i, 1);
                }
            }

            // ---- Rule 21: Z12.4 only stays when a pap smear is actually
            // present to link it to. Same code set the Q0091/G0101 linking
            // rule uses in al_buildCPTRules (Z12.4 is the link target for
            // Q0091, G0101, 88142, 88150, G0123, 3015F and S0612). ----
            const PAP_LINKABLE_CPTS = ['Q0091', 'G0101', '88142', '88150', 'G0123', '3015F', 'S0612'];
            const hasPapLinkableCpt = rawCPTCodesNow.some(c => PAP_LINKABLE_CPTS.includes(c)) ||
                Array.from(desired.keys()).some(c => PAP_LINKABLE_CPTS.includes(c));
            if (!hasPapLinkableCpt) {
                ...
                for (let i = toAdd.length - 1; i >= 0; i--) {
                    if ((toAdd[i].code || '').toUpperCase() === 'Z12.4') toAdd.splice(i, 1);
                }
            }
        }
    ...
    // ── Hasnayen payer helpers ─────────────────────────────────────────
    // Healthfirst sometimes renders as "Health First" (with a space) —
    // same normalization computeAnalysis's inline isHealthfirst check
    // already used; promoted to a shared helper so the Hasnayen-specific
    // Healthfirst rules (99401 60-day gap, G0447 60-day gap, 1159F/1160F
    // exclusion) all match the payer identically. (Hasnayen rules 3/4/25.)
    function isHealthfirstIns(insurance) {
        return !!insurance && /^health[\s-]*first\b/i.test(insurance.trim());
    }

    // Fidelis Care — rule 17: 99214 is never used for this payer.
    function isFidelisIns(insurance) {
        return !!insurance && /^fidelis\b/i.test(insurance.trim());
    }

    // Wellcare — rule 2: 99401 not applicable.
    function isWellcareIns(insurance) {
        return !!insurance && /^well\s*care\b/i.test(insurance.trim());
    }

    // CenterLight — rule 8: preventive AWV uses the Medicare G0438/G0439
    // codes, routed through the exact same first-vs-subsequent (new vs
    // established) branch straight Medicare and VNS Choice already use.
    function isCenterLightIns(insurance) {
        return !!insurance && /center\s*light/i.test(insurance);
    }

    // Hasnayen rule 2 — 99401 (Preventive Counseling) is NOT usable for:
    // Medicaid / New York State Medicaid, Medicare, Wellcare, Fidelis,
    // UHC (incl. UMR — isUHCInsurance already covers UMR), NYCE PPO,
    // Clover Health. It IS allowed for every other payer, MetroPlus
    // explicitly included (rule 10).
    function isHasnayenCounselingBlockedIns(insurance) {
        if (!insurance) return false;
        const name = insurance.trim();
        ...
        if (isCloverHealthIns(name)) return true;
        return false;
    }

    // ---- Commercial-insurance office-visit rule (Hasnayen rule 9) ----
    // Ported verbatim from clients/getwell/smartcoder.js
    // (isGetwellCommercialInsurance) — Hasnayen follows Getwell's
    // commercial-insurance logic, not Bronx's (Bronx removed its
    // commercial-payer exclusion entirely in v1.46).
    // When the payer is one of the listed commercial insurances AND a
    // Preventive visit code is on the chart, the office visit E&M code is
    // NOT billable alongside it and must be removed. If the same
    // commercial payer has NO Preventive code on the chart, the office
    // visit code is billed normally — this function only identifies the
    // payer; the preventive-or-not branching happens where it's used.
    // Reuses isUHCInsurance() for United Healthcare / UMR / Oxford.
    function isHasnayenCommercialInsurance(insurance) {
        if (!insurance) return false;
    ...
    // Hasnayen rules 3 + 4: the "recently used" gap for 99401 and G0447 is
    // 2 months (60 days) for Healthfirst and 1 month (30 days) for every
    ...
    // ── Hasnayen visit-type table (rule 6) ─────────────────────────────
    // Visit type comes straight from the appointment caption. Each entry
    // says how the visit behaves:
    //   behavior: 'counseling'  -> counseling codes (99401) may apply
    //             'preventive'  -> preventive visit type
    //             'none'        -> No PV / no counseling
    //             'excluded'    -> marked "X" on the rule sheet: skip
    //                              entirely, no E&M recommendation at all
    //   ov:       explicit office-visit CPT override for this visit type
    //   patient:  'new' for the new-patient E/M list, otherwise established
    //
    ...
    //   F/U         -> counseling
    //   URG         -> counseling
    //   Vaccine     -> No PV/counseling
    //   NP          -> preventive (new patient)
    //   Call back   -> X (excluded, no action)
    //   Walk in     -> counseling
    //   Same day    -> counseling
    //   F/U refill  -> counseling
    //   Televisit   -> No PV/counseling
    //   INJ         -> counseling
    ...
    const HASNAYEN_VISIT_TYPES = {
        ...
        'vaccine':    { behavior: 'none' },
        'np':         { behavior: 'preventive', patient: 'new' },
        'call back':  { behavior: 'excluded' },
        'walk in':    { behavior: 'counseling' },
        'same day':   { behavior: 'counseling' },
        'f/u refill': { behavior: 'counseling' },
        'televisit':  { behavior: 'none' },
        'inj':        { behavior: 'counseling' },
        'posthos':    { behavior: 'counseling' }
    };

    // Normalizes the caption text before looking it up in the table above:
    // lowercased, punctuation-insensitive on the "f/u" vs "f-u" vs "fu"
    // spellings, whitespace collapsed. A caption that isn't in the table
    // at all returns null (no rule-6 entry).
    function hasnayenVisitTypeInfo(visitType) {
        let v = String(visitType || '').toLowerCase().trim().replace(/\s+/g, ' ');
        if (!v) return null;
        // "F-U", "FU" and "F/U" are the same visit type on real captions.
        v = v.replace(/\bf[\s\-\/]*u\b/g, 'f/u');
        // Common caption spellings that map onto a table row.
        const ALIASES = {
            'con': 'televisit',
            'tel': 'televisit',
            'follow up': 'f/u',
        ...
        return HASNAYEN_VISIT_TYPES[v] || null;
    }

    // Kept for the office-visit E&M block below, which still branches on a
    // coarse category. 'excluded' (rule 6's "X") and unknown visit types
    // return null so no E&M is recommended at all.
    function classifyVisitType(visitType) {
        const info = hasnayenVisitTypeInfo(visitType);
        if (!info || info.behavior === 'excluded') return null;
        return info.patient === 'new' ? 'new' : 'established';
    }
    ...
    function al_buildCPTRules() {
        ...
        Object.assign(rules, {
            ...
            // Rule 30: 99407 gets the identical ICD linking as 99406.
            "99407": { type: "multiICD", icds: [["F17"], ["Z71.6"]] },
            ...
            "92227": { type: "startsWith", icds: ["E11","E10","E13","E08","E09"], fallback: "al_officeVisit" },
            ...
            // Hasnayen rule 23 (weekend/holiday) — same linking as Getwell.
    ...
    // Ported from clients/getwell/smartcoder.js — the dynamic age-restricted
    // CPT list spread into al_deleteUnwantedCodes's CPT set (Hasnayen rule 7
    // requires keeping "the same al_getAgeRestrictedCPTs() spread"; Bronx no
    // longer had this helper, so it comes back from the Getwell original,
    // unchanged, including the commented-out 1159F/1160F block).
    ...
    function al_deleteUnwantedCodes(done) {
        // Hasnayen rule 7 — this is the EXACT delete/prune list from the
        // client's rule doc, replacing Bronx's list wholesale. The
        // age-restricted CPT spread is preserved the way Getwell (the
        // client this AL module was originally ported from) does it —
        // Bronx had dropped al_getAgeRestrictedCPTs() entirely, so the
        // helper is reinstated below this function's callers.
        const cptsToDelete = new Set([
            'G9432', 'G8783', 'G9920', 'G9820', '4013F',
            'G9744', 'G9903', '4000F', '1034F', '3062F',
            '3725F', 'H0049', '99000', '99001',
            '3046F', 'H0001', 'G8754', 'G8752',
    ...
    // ─── Hasnayen-specific modifier-25 rules (rules 19, 28, 29) ────────
    // Reads an existing modifier value off a CPT row (Angular scope
    ...
    function al_getCPTModifier(row, field) {
        try {
            ...
            if (scope && scope.cpt && typeof scope.cpt[field] === 'string') {
                return scope.cpt[field].trim();
            }
        } catch (e) { /* fall through to the DOM path */ }
        const input = row.querySelector(`input[data-fieldname="${field}"]`) ||
                     row.querySelector(`input[name="${field}"]`) ||
                     row.querySelector(`input[id*="${field}"]`);
    ...
    // Puts modifier 25 on `row` in the first free modifier slot (mod1,
    // then mod2). If 25 is already there, or both slots are already
    // occupied by something else, the row is left untouched — never
    // overwrite a modifier the practice set deliberately (same
    // "if already applied then ok" convention as al_applyModifier59).
    function al_apply25Modifier(row) {
        if (!row) return;
        const mod1 = al_getCPTModifier(row, 'mod1');
        const mod2 = al_getCPTModifier(row, 'mod2');
        if (mod1 === '25' || mod2 === '25') return;
        if (!mod1) { al_setCPTModifier(row, 'mod1', '25'); return; }
        if (!mod2) { al_setCPTModifier(row, 'mod2', '25'); return; }
    }

    // The E/M code the 25 modifier belongs on: the office-visit CPT if one
    // is present, otherwise the preventive CPT (rules 28 + 29 both say
    // "office-visit or preventive CPT").
    function al_findEMRowForModifier25(cptRows) {
        const ovRow = cptRows.find(row => {
            const code = (row.querySelector('td:nth-child(2)')?.textContent.trim() || '').toUpperCase();
            return AL_OFFICE_VISIT_CODES.has(code);
        });
        if (ovRow) return ovRow;
        return cptRows.find(row => {
            const code = (row.querySelector('td:nth-child(2)')?.textContent.trim() || '').toUpperCase();
            return ALL_PREVENTIVE_EM_CODES.includes(code) || MEDICARE_AWV_CODES.includes(code);
        }) || null;
    }

    // Vaccine/injection administration codes — "immunization exists" for
    // rule 28. Reuses the existing VACCINE_ADMIN_CODE_UNIVERSE plus the
    // vaccine PRODUCT codes the SL-modifier rule already enumerates.
    const AL_IMMUNIZATION_CPT_CODES = new Set([
        ...VACCINE_ADMIN_CODE_UNIVERSE,
        "90380","90381","90382","90480","90589","90611","90619","90620","90621","90622","90623","90624","90633",
        "90647","90648","90651","90656","90657","90658","90660","90661","90671","90674","90677","90680","90681",
        "90686","90688","90696","90697","90698","90700","90702","90707","90710","90713","90714","90715","90716",
    ...
    // Q0091 (Pap smear collection) and G0101 (cervical/vaginal cancer
    // screening exam) — rule 29.
    const AL_PAP_SCREENING_CODES = new Set(['Q0091', 'G0101']);

    function al_applyHasnayen25Modifiers() {
        const cptRows = Array.from(document.querySelectorAll('#billingTbl4 tbody tr'));
        if (!cptRows.length) return;
        const codesPresent = cptRows
            .map(r => (r.querySelector('td:nth-child(2)')?.textContent.trim() || '').toUpperCase())
            .filter(Boolean);
        ...
        // ---- Rules 28 + 29: an immunization, or Q0091/G0101, puts a 25
        // modifier on whichever E/M code applies (office visit, else
        // preventive). The "no counseling code in that case" half of both
        // rules is enforced on the Analyze side (computeAnalysis).
        const hasImmunization = codesPresent.some(c => AL_IMMUNIZATION_CPT_CODES.has(c));
        const hasPapScreening = codesPresent.some(c => AL_PAP_SCREENING_CODES.has(c));
        if (hasImmunization || hasPapScreening) {
            al_apply25Modifier(al_findEMRowForModifier25(cptRows));
        }
    }
    ...
    function cl_buildCPTRules() {
        ...
        Object.assign(rules, {
            ...
            "93000": { type: "customICDCollector", icdList: ecgICDs, fallback: "cl_officeVisit" },
            "93005": { type: "customICDCollector", icdList: ecgICDs, fallback: "cl_officeVisit" },
            "93010": { type: "customICDCollector", icdList: ecgICDs, fallback: "cl_officeVisit" },
            ...
            "92227": { type: "startsWith", icds: ["E11","E10","E13","E08","E09"], fallback: "cl_officeVisit" },
    ...
    const cl_EXCLUDED_ICDS = new Set(["E66.9", "E66.01", "E66.09", "E66.3", "F17.210", "F17.200", "F17.220", "E55.9", "E56.9"]);
    ...
    // ================= WEEKEND / HOLIDAY RULE (CPT 99051) =================
    // Hasnayen rule 23 — ported verbatim from clients/getwell/smartcoder.js
    // (clients/hasan-sheikh/smartcoder.js carries the identical block).
    // Bronx had removed this feature in its v1.35, so everything below is
    // reinstated from the Getwell original: the holiday table, the DOS
    // detection, the per-patient/per-DOS manual override, and the toggle
    // rendered next to CURRENT ENCOUNTER in the panel.
    //
    ...
    function isTelevisitNow() {
        // Rule 6: "Televisit" is its own caption spelling for Hasnayen;
        // CON/TEL are the same visit type.
        return ['con', 'tel', 'televisit'].includes(getVisitType().toLowerCase().trim());
    }
    ...
    function computeQuickActionGating(insurance, flags, text) {
        ...
        // Hasnayen rule 3: the gap since the last use of 99401 is 2 months
        // (60 days) for Healthfirst and 1 month (30 days) for every other
        // payer that allows the code. The rest of the verification 99401
        // needs per rule 3 — at least one qualifying chronic ICD, and
        // vitals documented — is already enforced here
        // (hasChronicDiseaseThisEncounter below, and the isVitalsDocumented
        // sweep at the end of this function). computeAnalysis deletes an
        // already-present 99401 whenever this gating comes back disabled,
        // which is exactly rule 3's "if not met, remove it".
        // Rule 6: counseling codes only apply on a visit type classified
        // 'counseling' — 'none' (Blood Test / Vaccine / Televisit),
        // 'preventive' (ANN VISIT / NP) and 'excluded' (Call back) all
        // block P/C.
    ...
    async function runPreventiveAction() {
        ...
        try {
            ...
            // Only STRAIGHT/plain Medicare, VNS Choice and (Hasnayen rule
            // 8) CenterLight get the G0438/G0439 Medicare AWV codes. Every
            // other insurance — Clover Health included, and any Medicare
            // Advantage or other plan whose name merely mentions
            // "Medicare" — gets the age-banded 993xx preventive E&M code
            // instead, same as any other non-Medicare payer.
            const isStraightMedicare = isStraightMedicareIns(insurance);
            // Rule 8: CenterLight's preventive AWV codes are G0438/G0439,
            // picked by the exact same established-vs-new (subsequent vs
            // first) branch below that Medicare/VNS Choice already use.
            const isCenterLight = isCenterLightIns(insurance);
            ...
            if (isVNS || isStraightMedicare || isCenterLight) {
                // VNS Choice, straight Medicare and CenterLight are treated
                // the same: directly add G0438/G0439. Never G0402 (retired).
                ...
                const payerLabel = isVNS ? 'VNS Choice' : (isCenterLight ? 'CenterLight' : 'Medicare');
    ...
    // Hasnayen rule 2: 99401 (Preventive Counseling) can't be applied for
    // Medicaid / New York State Medicaid, Medicare, Wellcare, Fidelis,
    // UHC (incl. UMR), NYCE PPO and Clover Health. It CAN be applied for
    // every other payer — MetroPlus explicitly included (rules 2 + 10),
    // and Molina is no longer blocked (Bronx blocked it; rule 2's list
    // doesn't).
    ...
    async function runSmokingAction() {
        ...
        try {
            ...
            // Rule 30: 99406 is the default smoking-cessation code. If the
            // practice has already put 99407 (the longer session) on the
            // chart, it stays untouched and 99406 is NOT added alongside it.
            const has99407Already = getCPTRows().some(r =>
                (r.querySelector('td:nth-child(2)')?.textContent.trim() || '').toUpperCase() === '99407');
